...
class ImageOpenVINOPreprocessing():
    ...
    def draw_gaze_line(self, img, coord1, coord2, laser_flag):
        if laser_flag == False:
            # simple line
    ...
    def main(self, img):
        ...
        for obj in res_det[self.out_name_det][0][0]:                                # obj = [ image_id, label, conf, xmin, ymin, xmax, ymax ]
            if obj[2] > 0.75:                                                  # Confidence > 75%
                ...
                lm=res_lm[self.out_name_lm][0][:8].reshape(4,2)                      #  [[left0x, left0y], [left1x, left1y], [right0x, right0y], [right1x, right1y] ]
                ...
                for i in range(2):
                    ...
                    eyes.append(cv2.resize(face[y1:y2, x1:x2].copy(), (self.input_shape_gaze[self._W], self.input_shape_gaze[self._H])))    # crop and resize
                    ...
                    if roll != 0.:
                        rotMat = cv2.getRotationMatrix2D((int(self.input_shape_gaze[self._W]/2), int(self.input_shape_gaze[self._H]/2)), roll, 1.0)
                        eyes[i] = cv2.warpAffine(eyes[i], rotMat, (self.input_shape_gaze[self._W], self.input_shape_gaze[self._H]), flags=cv2.INTER_LINEAR)
        ...
        if self.spark_flag == True:
            for g1 in range(len(gaze_lines)):
                for g2 in range(g1+1, len(gaze_lines)):
                    ...
                    x2 = gaze_lines[g2][0]
                    y2 = gaze_lines[g2][1]
                    if self.intersection_check(x1, y1, x2, y2) == True:
                        l1 = self.line(x1, y1)
                        l2 = self.line(x2, y2)
                        x, y = self.intersection( l1, l2 )           # calculate crossing coordinate
